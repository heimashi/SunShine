...
public class GifDecoder {
    // Max decode bitmap size
    public final static int MAX_DECODE_SIZE = 1024 * 1024;
    ...
    /**
     * File read status: was cancelled
     */
    public static final int STATUS_DECODE_CANCEL = 3;

    // total decode bitmap size
    private long mDecodeBmSize;

    private long mMaxDecodeSize = MAX_DECODE_SIZE;

    // whether decode finishes at the end of the file
    private boolean mDecodeToTheEnd;

    // indicate how many frames are decoded
    private int mDecodedFrames;

    // indicate from which frame to decode
    ...
    private int[] dest; // last bitmap pixels

    private boolean mIsDecodeCertainFrame = false;

    public void setDecodeCertainFrame(int frame) {
        mStartFrame = frame;
        mIsDecodeCertainFrame = true;
    }
    ...
    protected void setPixels() {
        ...
        if (lastDispose > 0) {
            if (lastDispose == 3) {
                ...
                int n = getFrameCount() - 2;
                if (n > 0) {
                    Bitmap bm = getFrame(n - 1);
                    if (!bm.equals(lastBitmap)) {
                        lastBitmap = bm;
    ...
    // Once is canceled, it cannot be used to decode any more. It can be called
    // from another thread.
     ...
     */
    protected static int readOneByte(InputStream is) {
        int curByte = 0;
        try {
            curByte = is.read();
        } catch (Exception e) {
    ...
    /**
     * Detect whether the stream is a gif image, stream will be closed after
     * return.
     * 
    ...
    public static boolean isGifStream(InputStream is) {
        ...
        if (is != null) {
            String id = "";
            for (int i = 0; i < 6; i++) {
                int ch = readOneByte(is);
                if (ch == -1) {
                    break;
                }
                id += (char) ch;
            }
            ret = id.startsWith("GIF");
        }
        return ret;
    }
    ...
    /**
     * return real frame count. this is known only when decode to the end yet.
     * return: 0 if not decode to the end yet
     */
        ...
        while (!(done || err())) {
            ...
            switch (code) {
                case 0x2C: // image separator
                    // help check whether a frame is added or discarded
                    int framesBeforeRead = frames.size();

                    readBitmap();
                    if (mIsDecodeCertainFrame) {
                        if (!frames.isEmpty()) {
                            done = true;
                        }
                    }
                    if (image != null) {
                        if (frames.size() > framesBeforeRead) {
                            mDecodeBmSize += image.getRowBytes() * image.getHeight();
                        }
                    }
                ...
                case 0x3b: // terminator
                    ...
                    // indicate decode finishes at the end of the file
    ...
    protected void readBitmap() {
        ...
        // only store decoded frame when reach specified startFrame
